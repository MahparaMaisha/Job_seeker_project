...
//fetching posts
$stmt = $conn->prepare("SELECT jp.JobID AS JobID, jp.Title as Title, jp.JobType as JobType, jp.Description as Description, jp.Skills as Skills, jp.salary AS salary, COUNT(at.JobSeekerID) AS number_of_applicants, COUNT(CASE WHEN at.Status = 'Pending' THEN 1 END) AS Pending, COUNT(CASE WHEN at.Status = 'Accepted' THEN 1 END) AS Accepted, COUNT(CASE WHEN at.Status = 'Rejected' THEN 1 END) AS Rejected FROM jobPost jp LEFT JOIN application at ON jp.JobID = at.JobID where jp.RecruiterID=:rec_uid GROUP BY jp.JobID");
$stmt->execute([
